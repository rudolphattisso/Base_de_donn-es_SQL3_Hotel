...
--4 : Accordez les privilèges « SELECT », « INSERT », « UPDATE » et « DELETE »
-- à l’utilisateur « application_admin » sur toutes les tables sauf « station ».
...
--les requetes sont accessibles depuis le profil appliction_admin, sauf station.

--6. Créez un nouveau rôle « application_client » pouvant se connecter à votre base de données.
create role application_client login password '123456';

--7. Ajoutez les privilèges de lecture des données uniquement sur votre vue permettant de retrouver 
--chambres avec le nom de l’hôtel et le nom de la station (vue 2).
GRANT select 
ON   room_list
TO application_client;

--8. Testez vos permissions.
--les permissions sont bien contraintes.
